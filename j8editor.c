...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, PSTR sCmdLine, int iCmdShow) {
    ...
    while(GetMessage(&msg, NULL, 0, 0) > 0) {
        if(!IsDialogMessage(hDialogSearch, &msg)) {
...
void toClickMenuItem(HWND hWnd, WPARAM wParam) {
    ...
    case 221:
    case 321:
        toShowDialogSearch(TEXT_SEARCH);
        break;
    case 222:
    case 322:
        toSearchText();
    ...
    case 223:
    case 323:
        toShowDialogSearch(TEXT_REPLACE);
        break;
    case 230:
...
void toConvertAnsiFromUnicodeBe(wchar_t *wcUnicode, char *cAnsi) {
    ...
    toConvertAnsiFromUnicodeLe(wcUnicode, TRUE);
}
...
void toConvertAnsiFromUtf8(char *cUtf8) {
    ...
    toConvertAnsiFromUnicodeLe(wcUnicode, FALSE);
}
...
void toReadFile() {
    ...
    if(dReadSize > 0) {
        ...
        case CHAR_USC2BE:
            ...
            break;
        case CHAR_USC2LE:
            toConvertAnsiFromUnicodeLe((wchar_t *) cFileBuffer + 1, TRUE);
            break;
        }
    }
...
void toCreateDialog(HWND hParent) {
    hDialogGoto = CreateDialog((HINSTANCE)GetWindowLongPtr(hParent, GWLP_HINSTANCE),
                               MAKEINTRESOURCE(400),
                               hParent,
                               (DLGPROC)indexProc
                              );

    hDialogSearch = CreateDialog((HINSTANCE)GetWindowLongPtr(hParent, GWLP_HINSTANCE),
                                 MAKEINTRESOURCE(450),
                                 hParent,
                                 (DLGPROC)indexProc
                                );
...
void toGetLine(int iNumber) {
    ...
    int iLength = SendMessage(hEditor, EM_LINELENGTH, iNumber, 0);
    SendMessage(hEditor, EM_SETSEL, iStart, iStart + iLength);
}
...
BOOL toSearchOrCancel(HWND hWnd, WORD wId) {
    ...
    case IDCANCEL:
        ...
        return TRUE;
    case 402:
        GetDlgItemText(hWnd, 401, cBuffer8, sizeof(cBuffer8));
        toGetLine(atoi(cBuffer8) - 1);
        SendMessage(hDialogGoto, WM_COMMAND, IDCANCEL, 0);
        break;
    case 452:
        GetDlgItemText(hWnd, 451, cSearchString, sizeof(cSearchString));
        toSearchText();
        SendMessage(hDialogSearch, WM_COMMAND, IDCANCEL, 0);
        break;
    case 454:
        GetDlgItemText(hWnd, 451, cSearchString, sizeof(cSearchString));
        GetDlgItemText(hWnd, 453, cReplaceString, sizeof(cReplaceString));
        toReplaceText();
        SendMessage(hDialogSearch, WM_COMMAND, IDCANCEL, 0);
        break;
    default:
...
void toShowDialogGoto() {
    ...
    ShowWindow(hDialogGoto, SW_SHOW);
}
...
void toShowDialogSearch(int iSOR) {
    ...
    GetWindowRect(hDialogSearch, &rect);
    iDialogHeight = rect.bottom - rect.top;
    iDialogWidth = rect.right - rect.left;

    GetClientRect(hDialogSearch, &rect);
    iDialogHeight -= rect.bottom - rect.top;
    iDialogHeight += iSOR ? 75 : 40;

    MoveWindow(hDialogSearch,
               pScreen.x + (iParentWidth - iDialogWidth) / 2,
               pScreen.y + (iParentHeight - iDialogHeight) / 2,
               iDialogWidth,
               iDialogHeight,
    ...
    ShowWindow(GetDlgItem(hDialogSearch, 453), iSOR);
    ShowWindow(GetDlgItem(hDialogSearch, 454), iSOR);
    ShowWindow(hDialogSearch, SW_SHOW);
}